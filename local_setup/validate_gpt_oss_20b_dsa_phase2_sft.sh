...
if [[ "${MCORE_PHASE2_IN_CONTAINER:-0}" != "1" ]]; then
    ...
    CONTAINER_SFT_DATA_PATH=$SFT_DATA_PATH
    if [[ "$SFT_DATA_PATH" == "$REPO_ROOT/"* ]]; then
        CONTAINER_SFT_DATA_PATH="/workspace/megatron-lm/${SFT_DATA_PATH#"$REPO_ROOT/"}"
    fi
    exec env MODE=exec ARTIFACT_ROOT="$ARTIFACT_ROOT" IMAGE_TAG="$IMAGE_TAG" \
        ...
        SFT_DATA_PATH="$CONTAINER_SFT_DATA_PATH" \
        RUN_NAME="$RUN_NAME" \
        RUN_DIR="$RUN_DIR" \
        IMAGE_TAG="$IMAGE_TAG" \
...
SAVE_CHECKPOINT="$SAVE_CHECKPOINT" TRAIN_ITERS="$TRAIN_ITERS" SEQ_LENGTH="$SEQ_LENGTH" \
    bash local_setup/run_gpt_oss_20b_dsa_phase2_sft.sh \
